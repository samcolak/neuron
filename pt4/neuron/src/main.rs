...
use helpers::brain::QuestionStoreDecision;
...
fn build_multimodal_demo_network() -> MultiModalNeuralNetwork {
    ...
    network.insert_text("machine reasons", "en", DendriteType::Statement);
    network.absorb_true_text("water is wet", "en", DendriteType::Statement);
    network.absorb_true_text("gravity pulls", "en", DendriteType::Statement);
    network.absorb_true_text("fire is hot", "en", DendriteType::Statement);
    ...
    network.insert_image_bytes(&signal_image, "img", DendriteType::Token);
    ...
    network.absorb_truth(&sensor_features, "sensor", DendriteType::Token);
    ...
    network.insert_multimodal(&vision_features, "vision", DendriteType::Token);
...
fn run_model_checks(network: &MultiModalNeuralNetwork) -> Vec<ModelCheck> {
    ...
    let checks = vec![
        ...
        ModelCheck {
            ...
            actual: network.enumerate_multimodal_path(&sensor_features).0.is_some(),
            expected: true,
        },
        ModelCheck {
            label: "feature-token vision lookup",
            actual: network.enumerate_multimodal_path(&vision_features).0.is_some(),
            expected: true,
        },
        ModelCheck {
            label: "partial question returns a usable score",
            actual: (network.evaluate_question_fuzziness(&partial_question) - 0.75).abs() < f64::EPSILON,
            expected: true,
        },
        ModelCheck {
        ...
        ModelCheck {
            ...
            actual: network.decide_question_storage(&partial_question) == QuestionStoreDecision::Store,
            expected: true,
        },
        ModelCheck {
            label: "unknown question should defer",
            actual: network.decide_question_storage(&unknown_question) == QuestionStoreDecision::Defer,
            expected: true,
        },
        ModelCheck {
...
fn run_multimodal_demo() {
    ...
    let passed = checks.iter().filter(|check| check.actual == check.expected).count();

    println!("  model checks: {}/{} passed", passed, checks.len());
    for check in checks {
        println!(
            "    [{}] {}",
            if check.actual == check.expected { "PASS" } else { "FAIL" },
            check.label
        );
    }
...
mod tests {
    ...
    fn demo_network_keeps_learning_and_truth_separate() {
        ...
        assert!(network.enumerate_image_bytes_path(&vec![64u8; 128]).0.is_some());
        assert!(network.enumerate_multimodal_path(&MultiModalInput::FeatureTokens {
            modality: "sensor".to_string(),
            tokens: vec!["temp:1f".to_string(), "humidity:08".to_string()],
        }).0.is_some());
        assert!(network.enumerate_multimodal_path(&MultiModalInput::FeatureTokens {
    ...
    fn demo_network_scores_known_partial_and_unknown_questions() {
        ...
        let fuzzy = network.evaluate_question_fuzziness(&MultiModalInput::Text("cat on mat extra".to_string()));
        let unknown = network.evaluate_question_fuzziness(&MultiModalInput::Text("does not exist".to_string()));

        assert_eq!(fuzzy, 0.75);
        assert_eq!(unknown, -1.0);
        assert_eq!(network.decide_question_storage(&MultiModalInput::Text("cat on mat extra".to_string())), helpers::brain::QuestionStoreDecision::Store);
        assert_eq!(network.decide_question_storage(&MultiModalInput::Text("does not exist".to_string())), helpers::brain::QuestionStoreDecision::Defer);
    }
    ...
    fn demo_network_rejects_empty_or_unsupported_inputs() {
        ...
        assert_eq!(network.decide_question_storage(&MultiModalInput::Text(String::new())), helpers::brain::QuestionStoreDecision::Defer);
        assert_eq!(network.decide_question_storage(&MultiModalInput::FeatureTokens {
            modality: "unknown".to_string(),
            tokens: Vec::new(),
        }), helpers::brain::QuestionStoreDecision::Defer);
    }
