    ...
    match network.snapshot_instance(snapshot_id) {
        Ok(()) => {
            ...
            match restored.load_snapshot_instance(snapshot_id) {
                Ok(status) => {
                    println!(
                        "  snapshot[{}]: saved and loaded (cognitive_loaded={}, memory_loaded={}, classifier_loaded={})",
                        snapshot_id,
...
mod tests {
    use super::*;
    use neuralnet::helpers::pattern_classifier::PatternClassifier;
    ...
    fn demo_network_scores_known_partial_and_unknown_questions() {
        ...
        let fuzzy =
            network.evaluate_question_fuzziness(&MultiModalInput::Text("cat on mat extra".to_string()));
        let unknown =
            network.evaluate_question_fuzziness(&MultiModalInput::Text("does not exist".to_string()));
    ...
    #[test]
    fn demo_classifier_layer_predicts_known_labels() {
        ...
        let text_result = classifier.classify(&MultiModalInput::Text("cat on mat".to_string()));
        assert!(text_result.is_some());
        let text_prediction = text_result
            .unwrap_or_else(|| panic!("classifier should produce text label"));
        assert_eq!(text_prediction.label, "animal_cat");
        assert!((text_prediction.score - 1.0).abs() < f64::EPSILON);

        let sensor_result = classifier.classify(&MultiModalInput::FeatureTokens {
            modality: "sensor".to_string(),
            tokens: vec!["temp:1f".to_string(), "humidity:08".to_string()],
        });

        assert!(sensor_result.is_some());
        let sensor_prediction = sensor_result
            .unwrap_or_else(|| panic!("classifier should produce sensor label"));
        assert_eq!(sensor_prediction.label, "sensor_hot");
        assert!((sensor_prediction.score - 1.0).abs() < f64::EPSILON);

        let unknown_result = classifier.classify(&MultiModalInput::Text("bird in sky".to_string()));
        assert!(unknown_result.is_none());

        let ranked = classifier.classify_top_k(&MultiModalInput::Text("cat on mat".to_string()), 3);
