use crate::helpers::controllers::image_controller::ImageNodeController;
use crate::helpers::image_dendrite::ImageDendrite;
use crate::helpers::image_io::{load_png_or_jpeg_from_path, ImageByteBuffer, ImageIoError};
...
impl NeuralNetwork<ImageNodeController, ImageDendrite> {
    ...
    pub fn enumerate_image_path(&self, image_bytes: &[u8]) -> (Option<ImageDendrite>, Vec<ImageDendrite>) {
        self.enumerate_path_content(image_bytes)
    }

    pub fn enumerate_image_buffer_path(&self, image_buffer: &ImageByteBuffer) -> (Option<ImageDendrite>, Vec<ImageDendrite>) {
    ...
    pub fn insert_image(&mut self, image_bytes: &[u8], language: &str, dendrite_type: DendriteType) {
        self.insert_content(image_bytes, language, dendrite_type)
    }

    pub fn insert_image_buffer(&mut self, image_buffer: &ImageByteBuffer, language: &str, dendrite_type: DendriteType) {
        self.insert_content(image_buffer.as_slice(), language, dendrite_type)
    }
