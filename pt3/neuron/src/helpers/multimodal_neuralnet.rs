...
pub type MultiModalNeuralNetwork = NeuralNetwork<MultiModalController, MultimodalDendrite>;
...
impl NeuralNetwork<MultiModalController, MultimodalDendrite> {
    ...
    pub fn insert_text(&mut self, text: &str, language: &str, dendrite_type: DendriteType) {
        self.insert_content(&MultiModalInput::Text(text.to_string()), language, dendrite_type);
    }

    pub fn enumerate_text_path(&self, text: &str) -> (Option<MultimodalDendrite>, Vec<MultimodalDendrite>) {
        self.enumerate_path_content(&MultiModalInput::Text(text.to_string()))
    }

    pub fn insert_image_bytes(&mut self, bytes: &[u8], language: &str, dendrite_type: DendriteType) {
        self.insert_content(&MultiModalInput::ImageBytes(bytes.to_vec()), language, dendrite_type);
    }

    pub fn enumerate_image_bytes_path(&self, bytes: &[u8]) -> (Option<MultimodalDendrite>, Vec<MultimodalDendrite>) {
        self.enumerate_path_content(&MultiModalInput::ImageBytes(bytes.to_vec()))
    }
...
mod tests {
    ...
    #[test]
    fn single_network_accepts_text_and_image_content() {
    ...
    #[test]
    fn feature_token_mode_supports_future_modalities() {
        let mut network = MultiModalNeuralNetwork::new_multimodal();
