...
use crate::helpers::dendrite::{Dendrite, DendriteType};
...
pub fn neuralnet_add_dendrite(data: &str, language: &str, dendrite_type: DendriteType) {
    ...
    let dendrite = Dendrite::new(data, language, dendrite_type);
    ...
    pub fn insert(&mut self, content: &str, language: &str, dendrite_type: DendriteType) {
        ...
        for index in 0..tokens.len() {
            ...
            if is_stop_word(&token_keys[index]) {
                ...
                let new_dendrite = Dendrite::new(&tokens[index], language, DendriteType::StopWord);
            ...
            let uid = match selected {
                ...
                None => {
                    let new_dendrite = Dendrite::new(&tokens[index], language, dendrite_type);
                    let new_uid = new_dendrite.uid.clone();
...
mod tests {
    ...
    fn seeded_network(entries: &[(&str, &str)]) -> NeuralNetwork {
        ...
        for (uid, data) in entries {
            let mut dendrite = Dendrite::new(*data, "en", DendriteType::Token);
